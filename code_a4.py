...
print("Question 1a")
P_min, a_opt, _ = do_grid_search(F, G, R, n_inputs, a_range=[0.01, 1, 100])
...
b_ = np.array([[3]])
Yinv_min, a_opt, Rhat_opt = do_grid_search(F, G, R, n_inputs, a_range=[0.64, 0.65, 1], constraints=True, c=c_, b=b_)
...
evs_sys1 = np.linalg.eigvals(F-G@K1)
evs_sys2 = np.linalg.eigvals(F-G@K2)

plt.figure(10)
plt.scatter([np.real(ev) for ev in evs_sys1], [np.imag(ev) for ev in evs_sys1], label="System 1")
plt.scatter([np.real(ev) for ev in evs_sys2], [np.imag(ev) for ev in evs_sys2], label="System 2")
t = np.linspace(0., np.pi * 2, 100)
plt.plot(np.cos(t), np.sin(t), "g", label="stability margin")
plt.xlabel(r"$\Re (e)$")
plt.ylabel(r"$\Im (e)$")
plt.xlim([-1.1, 1.1])
plt.ylim([-1.1, 1.1])
plt.grid(True)
plt.legend()
plt.savefig("./figures4report/a4_question3_closed-loop-evs.png")
plt.show()
